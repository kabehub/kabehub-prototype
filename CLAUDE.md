...
タスクを依頼するときは以下の3点セットで書く。曖昧な依頼はこの形式に沿って補完する。
...
```
Goal: チャット送信後にスレッドタイトルを自動生成する
Constraints:
  - 既存の Promise Bridge パターンを崩さない
  - タイトル生成APIの呼び出しはストリーミング完了後に行う
Acceptance criteria:
  - 新規スレッドの初回送信後、サイドバーのタイトルが「無題」から生成タイトルに更新される
  - 既存スレッドのタイトルは変更されない
  - ローカル・本番両方で動作する
```
