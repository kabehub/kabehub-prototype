-- migration_v189_project_memory_phase_e_merge_project_guard.sql
-- Project Memory Manager Phase E-2:
-- merge_user_edited_lore_pairで異なるproject_idのLore同士を統合できないようにする。
-- migration_v188適用後、呼び出し元をproject_id版RPCへ切り替える前に適用する。
